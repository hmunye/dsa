...
pub fn gcd(mut x: usize, mut y: usize) -> usize {
    ...
    while y > 0 {
        ...
        // this iteratively, we progressively reduce the problem. Each iteration
        // brings us closer to the base case where `y` becomes 0. When `y`
        // becomes 0, the current value of `x` will be the GCD.
        let tmp = x;
        x = y;
        y = tmp % y;
    }
